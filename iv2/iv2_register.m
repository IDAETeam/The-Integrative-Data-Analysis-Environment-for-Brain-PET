...
% ???
h                               = findobj(0,    'Tag',      [mfilename,'_sid']);   
...
q25                             = zeros(max(q24(:))+1,      length(im1));
for i=0:1:max(q24);
    q26                         = umo_cstrs(c(1).mat,[int2str(i),' '],  'im1');
    for j=1:1:length(im1);
        k                       = find(q26>im1(j) & q26<im1e(j));
        if length(k)==1;        q25(i+1,    j)              = q26(k);
        elseif length(k)>1;     q25(i+1,    j)              = nan;                  end;    end;    end;
%
if any(isnan(q25(:)));
...
for i=1:1:size(q25,2);
    if q25(1,i)>0;              mmm{i}                      = deblank(c(2).mat(q25(1,i),:));
    else;                       mmm{i}                      = m00;                                  end;
    for j=q24';
        if q25(j+1,i)>0;        ppp{i,j}                    = deblank(c(2).mat(q25(j+1,i),:));
        else;                   ppp{i,j}                    = p00;                  end;    end;    end;
%
%
im1                             = umo_cstrs(c(1).mat,'snm ','im1'); 
...
%         if umo_cstrs(char(fnms),['m4p',int2str(i)],'im1')>0;
%             m4pi                = ['m4p',int2str(i)];
%             eval(['out          = dd.',m4pi,';']);
%             out                 = local_dir_mri(out, m4pi, deblank(c(2).mat(lds(1),:)), i2);
%             if isempty(out);    ok                          = 0;
%             else;               eval(['dd.',m4pi,'          = out;']);                              end;    
%         else;
%             disp(['.problem! m4pet_',int2str(i),' is missing or wrong m2p filed.']);
%             ok                  = 0;                                                        end;    end;
if ok<1;    disp(['> aborting: ',mfilename,' for above reasons']);                  return;         end;
%
dx                              = local_dir(mmm,ppp,        deblank(c(2).mat(lds(1),:)),i2);
% dispCharArrays(1,dx.mri);
% for j=q24';                     disp([' pet',int2str(j),': ']);
%                                 dispCharArrays(1,dx.pet{j});                                        end;
...
%%
stm                             = feval(lds,                'stm',[]);
str                             = feval(lds,                'str',i2);
add                             = feval(lds,                'add',i2);
% sorting out mri directories:
disp('.sorting out MRI & PET directories: ');
m0                              = char(mmm);
% disp(char(mmm));
% when any mmm starts with '?' > to replace with the dammy folder:
if any(m0(:,1)=='?');
    mdx_dmy                  	= feval(lds,                'dmy',0);
    for i=find(m0(:,1)'=='?');  mmm{i}                      = mdx_dmy;                              end;
    m0                          = char(mmm);                                                        end;
%
dx.mri                          = m0(:,     size(stm,2)+2:end);
dx.mri(dx.mri=='/' | dx.mri=='\')                           = filesep;

mdxOK                           = ones(size(dx.mri,1),      1);
if ~isempty(str);
    str.mri                     = str.mri(1,                size(stm,2)+2:end);
    if ~isempty(add.mri);       ss                          = findstr(str.mri,[filesep,add.mri]);
                                str.mri                     = str.mri(1,    1:ss(end)-1);           end;
    s0                          = length(find(str.mri==filesep));
    for i=1:1:size(dx.mri,1);
        ss                      = find(dx.mri(i, :)==filesep);
        if length(ss)>s0;       dx.mri(i,   ss(s0+1):end)   = ' ';
        elseif length(ss)<s0;   mdxOK(i,    :)              = 0;                            end;    end;
    if any(~mdxOK);             ii                          = find(~mdxOK);
                                disp(' MRI path too short for some subjects');
                                disp(['  expeted: ',str.mri]);
                                dispCharArrays(1,dx.mri(ii,:),2,int2str(ii));                       end;
    d.mri                       = deblank(dx.mri);                                                  end;
if ~any(~mdxOK);                disp(' MRI directories: OK');                                       end;
%
if ~isempty(str);               str.pet                     = str.pet(1,    size(stm,2)+2:end);
    if ~isempty(add.pet);       ss                          = findstr(str.pet,[filesep,add.pet]);
                                str.pet                     = str.pet(1,    1:ss(end)-1);           end;
                                s0(:)                       = length(find(str.pet==filesep));       end;
%
nc                              = size(ppp, 2);
pdxOK                           = zeros(size(ppp,1),        nc);
pdx_dmy                         = feval(lds,                'dmy',1);
for j=1:1:nc;
    p0                          = char(ppp(:,   j));
    if any(p0(:,1)=='?');
        for k=find(p0(:,1)'=='?');
    ...
    dx.pet{j}                   = p0(:,     size(stm,2)+2:end);
    dx.pet{j}(dx.pet{j}=='/' | dx.pet{j}=='\')              = filesep;
    if ~isempty(str);           
        pdxOK(:,    j)          = 1;
        for i=1:1:size(dx.pet{j},1);
            ss                  = find(dx.pet{j}(i, :)==filesep);
            if length(ss)>s0;   dx.pet{j}(i, ss(s0+1):end)  = ' ';
            elseif length(ss)<s0;   
                                pdxOK(i,    j)              = 0;                            end;    end;
        if any(~pdxOK(:,j));      ii                        = find(~pdxOK(:,j));
                                disp([' PET path too short for condition: ',int2str(j)]);
                                disp(['  expeted: ',str.pet]);
                                disp([dx.pet{j}(ii,:),char(zeros(size(ii))+32),int2str(ii)]);       end;
                                                                                                    end;
        dx.pet{j}               = deblank(dx.pet{j});                                               end;
%
if ~any(~pdxOK(:));             disp(' PET directories: OK');                                       end;
if any(~mdxOK) | any(~pdxOK(:));
    disp('.revise pet / mri directory lines as above and resubmit');
    dx                          = [];                                                               end;
return;
%%
