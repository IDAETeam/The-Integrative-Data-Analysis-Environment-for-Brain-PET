...
%                                 y                           = load(ooo{1});
%         if isfield(y,'s4mpe');
%             im1                	= umo_cstrs(x.s4mpe.ffg, y.s4mpe.ffg(y.p4maps.mnos, :), 'im1');
%             x.p4maps.mnos    	= im1(im1>0);
%             x.p4maps.pet      	= x.s4mpe.pet(x.p4maps.mnos,    :);
%             x.p4maps.fwhm      	= y.p4maps.fwhm;
%             ic                	= 0;
%             for i=x.p4maps.mnos';   
%                 ic                	= ic + 1;
%                 [idx, inm]    	= fileparts(x.s4mpe.ffg(i, :));
%                 map{ic}        	= [inm,'_',x.s4mpe.ext_flg{i}{5},'.pvm'];
%                 ffg{ic}       	= [inm,'_',x.s4mpe.ext_flg{i}{5},'_pvm.ezd'];                       end;
%             x.p4maps.map       	= char(map);
%             x.p4maps.ffg      	= char(ffg);                                                end;    end;
% else;                           x                           = load(iii{1});                         end;
x.job                           = 'map2gen';
x.fbc                           = fbc(1, 1:3);
x.s4mpe.s4mpe                   = iii{1};
...
% return;
% ok2mvon                         = 1;
% if p2m(k).dnum<mv2_get_dnum([]);
%     disp('.not ready (PET2MRI coreg has not been done yet)');
%     ok2mvon                     = 0;
% else;
%     [f1, g1]                    = mv2_genfln(p2m(k).avr,    fbc(1, 1:3));
%     if g1<1;                    disp('.??? unable to locate PET file used for PET2MRI coreg');
%                                 disp([' sought: ',f1]);
%                                 ok2mvon                         = 0;                                end;
%     %
%     [tar_mri, g1]               = abc;
%     else;
%         if ~strcmpi([pnm, pex],p2m(k).pet);
%             % umo_cstrs(pnm, mp.p2m(k).pet, 'im1')<1;
%                                 disp('.??? scan orders have been modified?');
%                                 disp([' expected: ',mp.p2m(k).pet]);
%                                 disp([' observed: ',g4dxs.psid{k}(fbc(2), :)]);
%                                 ok2mvon                         = 0;                        end;    end;
%     mrino                       = m4pets(k);
%     [tar_mri, g1]               = mv2_genfln(mm.m2m(mrino).ffg, [fbc(1, 1:2),1]);
%     disp([' >matching MRI for PET #',int2str(k),': ',mm.m2m(mrino).ffg,' (MRI #',int2str(mrino),')'])
%     if g1<1;                    disp('.??? unable to locate matching MRI:');
%                                 ok2mvon                         = 0;                        end;    end;
%
% if ok2mvon<1;                   disp(['.leaving ',mfilename,' for above reasons']); return;         end;   
%
ic                              = 0;
...
for i=find(x.p4maps.pet(:, fbc(3))>0)';
    ...
    % disp([' map approach #',int2str(ic),': ',x.s4mpe.ext_str{k}]);
    ...
    if ~c0;                     map4sn{ic}                  = ' ';
        ...
        % functional volumes in PET space (*.pvm):
        ...
        if g3>0 && mv2_get_dnum({f3})<mv2_get_dnum({f1});
                                g3                          = 0;                                    end;
%         if g3>0 && ~strcmpi(m2m_old(mrino).ffg,mm.m2m(mrino).ffg);
%             disp(' .deleting previous functional volume (wrong matching MRI)');
%             delete(f3);
%             g3                  = 0;                                                                end;
