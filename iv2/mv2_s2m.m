...
out.methods                     = {'OTCM','TTCM3','TTCM4','TTCMC4','PRGA','BPITp','MA1'};   %,'TPR'};
...
cb.str{4}                       = {'K1=0.3, k2=0.2','K1=, k2=, k3=','OTCM+PRGA',    ...
                                    'ref','-','-','-'}; % ,'-'};
%
% for TTCM4 and TTCMC4, flg{4,i} will be 'OTCM&PRGA' 
%
return;
%%
...
%% 
% iflg = p4mpe.flg{tim_row, cNo}
% ires = p4mpe.res{tim_row, cNo}
...
for i=1:1:size(iflg,1);         
    set(bHs(i), 'Value',1,  'Style','pushbutton',   'String',deblank(iflg(i, :)));                  end;
%
...
t2                              = [',',int2str(umo_cstrs(char(cwUD.str4cb), 'time', 'im1'))];
% When clicked GUI displays * (Status #1):
if any(s0=='*');
    ...
    s1                          = str2num(s0);
    if cwUD.cbstr{umo_cstrs(char(cwUD.str4cb), 'tim ', 'im1')}{ umo_cstrs(char(cwUD.cbstr{1}),  ...
     	[get(findobj(gcf, 'Tag','prep.MPE.cmethod'), 'String'),' '], 'im1')}(1)=='0';
                                s1(2)                       = s1(2) + 5;
    else;                       s1(1)                       = s1(1) + 5;                            end;
    set(h0,   	'String',[int2str(s1(1)),'T',int2str(s1(2))],   'Style','edit',     ...
                                'BackgroundColor',iv2_bgcs(11));                    return;         end;
% 
set(gco,    'Style','pushbutton',   'BackgroundColor',iv2_bgcs(0));
ss                              = zeros(12, 2);
for i=1:1:12;
    s0                          = get(findobj(gcf, 'Tag',[int2str(i),t2]), 'String');
    s0(lower(s0)~=upper(s0)) 	= ' ';
    if size(str2num(s0),2)==2;  ss(i,   :)                  = str2num(s0);                  end;    end;
% 

ic                              = 0;
for i=find(ss(:,2)'>0);
    ic                          = ic + 1;
    set(findobj(gcf, 'Tag',[int2str(ic),t2]), 'String',[int2str(ss(i,1)),'T',int2str(ss(i,2))]);    end;
% 
% just in case none of those entered are not OK:
if ic<1;                        
    if strcmpi(cwUD.p4mpe.mflg,'RTMs');
                                set(findobj(gcf, 'Tag',['1',t2]), 'String','20T90');
    else;                       set(findobj(gcf, 'Tag',['1',t2]), 'String','0T90');         end;    end;
        
% adding * GUI to accept more time flags:
ic                              = ic + 1;
set(findobj(gcf, 'Tag',[int2str(ic),t2]), 'String','*', 'CallBack','mv2_s2m(''get_tim'',[]);');
%
% disabling the rest of GUIs:
for i=ic+1:1:12;
    set(findobj(gcf, 'Tag',[int2str(i),t2]), 'String',' ', 'CallBack',' ');                         end;
return;
%%
...
%
%
mNo                             = umo_cstrs(char(cwUD.cbstr{1}),            ...
...
%%
ud                              = get(gco,      'UserData');
h                               = findobj(gcf, 'Tag','prep.MPE.mflg');
[qqq, cb]                       = feval(['local_',lower(get(h, 'String')),'_var']);
k                               = umo_cstrs(char(qqq.str4cb),'gin ',    'im1');
m                               = umo_cstrs(char(qqq.methods),  ...
                                    [get(findobj(gcf, 'Tag','prep.MPE.cmethod'),'String'),' '], 'im1');
% cb.str{k}{m}
% if not in 'edit' style > activate the GUI:
if ~strcmpi(get(gco, 'Style'),'edit');
    % to modify (upper; to display current) or new (lower):
    if isfield(ud,'flg');       set(gco,    'Value',1,  'Style','edit', 'String',ud.flg);
    else;                       set(gco,    'Value',1,  'Style','edit', 'String',cb.str{k}{m});     end;
    set(findobj(gcf, 'Tag','prep.MPE.inst'),    'String',cb.title{k});              return;         end;
%
str                             = strip(get(gco,  'String'));
% enter none to delete it:
if isempty(str);                
    if isfield(ud,'flg');       str                         = [ud.flg,' (deleted)'];
    else;                       str                         = 'deleted';                            end;
    set(gco,    'Value',1,  'Style','pushbutton',   'String',str,   'UserData',[]); return;         end;
% 
if ~isempty(strfind(upper(str),'OTCM'));
    ud.flg                      = 'OTCM&PRGA';
    ud.res                      = -1; 
    set(gco,    'Value',1,  'Style','pushbutton',   'String',ud.flg,    'UserData',ud);
    %
    tNos                     	= str2num(get(gco,  'Tag'));
    set(findobj(gcf, 'Tag',[int2str(tNos(1)+1),',',int2str(tNos(2))]),  'String','*',   ...
                                'CallBack','mv2_s2m(''get_gin'',[]);');           	return;         end;
%
str(str==',' | str==';' | str=='=')                         = ' ';
cc                              = getLseg(str,  [0,1]);
s2                              = cb.str{k}{m};
...
c2                              = getLseg(s2,   [0,1]);
notok                           = 0;
if size(cc,1)~=size(c2,1)*2;    notok                       = 1;                                    end;
res                             = zeros(1,  size(c2,1));
for i=2:2:size(cc,1);
    if ~isempty(str2num(cc(i,:)));
                                res(1, i/2)                 = str2num(cc(i,:));             end;    end;
%
if any(res<10^-6);              notok                       = 1;                                    end;
% missing K1 etc or re-arranged orders:
im1                             = umo_cstrs(cc, c2, 'im1')
if any(~im1) || any(abs([1:2:size(cc,1)]'-im1(:,1))>0);
                                notok                       = 1;                                    end;
% 
% when the input is not right:
if notok>0;
    set(findobj(gcf, 'Tag','prep.MPE.inst'),    'String',['Just add numbers to: ',cb.str{k}{m}],    ...
                                'BackgroundColor',iv2_bgcs(11));
 	pause(1);
    set(findobj(gcf, 'Tag','prep.MPE.inst'),    'BackgroundColor',iv2_bgcs(4));     return;         end;
%
flg                             = [deblank(c2(1, :)),'=',num2str(res(1))];
for i=2:1:size(c2,1);
    flg                         = [flg,',',deblank(c2(i, :)),'=',num2str(res(i))];                  end;
%
ud.flg                          = flg;
ud.res                          = res;
%
...
%     % blank input > to remove:
%     if ~any(get(gco, 'String')~=' ');                    	local_blank_input;      return;         end;
%     c1                          = str2num(get(gco, 'String'));
%     if length(c1)~=1;           set(gco, 'String',['error: ',get(gco, 'String')]);  return;         end;
%     ud                          = struct('flg',num2str(c1),     'res',c1);
%     % saving the input:
%     set(gco,    'Style','pushbutton',   'UserData',struct('flg',num2str(c1),     'res',c1));
%     % activating the next GUI
%     set(findobj(gcf, 'Tag',[int2str(ww(1)+1),',',int2str(ww(2))]),      ...
%         'String','*',   'CallBack','mv2_s2m(''get_k2r'',[]);');                 	return;         end;
% return;
%
% ud                              = get(gco,      'UserData');
% [qqq, cb]                       = feval(['local_',lower(get(findobj(gcf, 'Tag','prep.MPE.mflg'),    ...
%                                                             'String')),'_var']);
% m                               = umo_cstrs(char(qqq.methods),  ...
%                                     [get(findobj(gcf, 'Tag','prep.MPE.cmethod'),'String'),' '], 'im1');
% if isfield(ud,'flg');           set(gco,    'Value',1,  'Style','edit', 'String',ud.flg);
% else;                           set(gco,    'Value',1,  'Style','edit', 'String',cb.str{ww(2)}{m});	end;
% set(findobj(gcf, 'Tag','prep.MPE.inst'),    'String',cb.title{ww(2)});       
return;
%%
...
% [c1, c2]                       	= getLseg(s0, 1);
% c1(c1==',')                     = ' ';
% im2                             = umo_cstrs(['   ';'*  ';'-  ';'k1=';'otc'],  lower(c1), 'im1');
% if any(im2<1);                  local_blink_co(ibHs(im2<1));                        return;         end;
% % when the string starts with - > not applicable:
% if max(im2)==3;                 res                         = nan(1);
%                                 flg                         = '-';                  return;         end;
% %
% if max(im2)==5;                 res                         = nan(1);
%                                 flg                         = 'OTCM+PRGA';         	return;         end;
% %
% res                             = nan(sum(im2==4),  2);
% ic                              = 0;
% for i=find(im2==4);             ic                          = ic + 1;
%                                 k1                        	= [];
%                                 k2                        	= [];
%                                 eval([lower(c1(i, :)),';']);
%                                 eval([lower(c2(i, :)),';']);
...
%                                 res(ic, :)                  = [k1, k2];
%                                 ffg{ic}                     = ['K1=',num2str(k1),', k2=',num2str(k2)];
%     else;                       c1(i,   :)                  = ' ';                          end;    end;
% % 
% if any(im2==4 & c1(:,1)==' ');  local_blink_co(ibHs(im2==4 & c1(:,1)==' '));    
%                                 res                         = [];                   return;         end;
% % 
% if any(isnan(res(:)));          res                         = [];
% else;                           flg                        	= char(ffg);                            end;
...
% % called from expand mode:
% %   
% % checking if all cells of 1st line are completed:
% ok                              = 1;
% %       
% cflg{1,1}                       = get(findobj(gcf, 'Tag','prep.MPE.cmethod'),   'String');
% cres{1,1}                       = cflg{1};
% for i=2:1:size(bHs,2)-1;
%     c1                          = getLseg(char(get(bHs(:,i),'String')), 1);
%     ic                          = 0;
%     res                         = [];
%     flg                         = ' ';
%     for j=find(c1(:,1)~=' ')';
%         ud                      = get(bHs(j,i), 'UserData');
%         if ~isempty(ud) && isfield(ud,'flg');
%             ic                  = ic + 1;
%             flg                 = char(flg, strtrim(ud.flg));
%             if ic==1;           res                         = ud.res;
%             else;               res                         = [res; ud.res];     	end;    end;    end;
%     if ~ic;                     ok                          = 0;   
%     else;                       cflg{i,1}                   = flg(2:end,    :);
%                                 cres{i,1}                   = res;                          end;    end;
% % not all needed top line GUIs are done:
% if ok<1;
%     set(findobj(gcf, 'Tag','prep.MPE.inst'),    'String','Complete columns with * at top line',     ...
%                                 'BackgroundColor',iv2_bgcs(11));
%  	pause(1);
%     set(findobj(gcf, 'Tag','prep.MPE.inst'),    'BackgroundColor',iv2_bgcs(4));     return;         end;
% % entering/revising PETs to apply this method:
% pHs                             = get(findobj(gcf, 'String','PETs'),    'UserData');
% pets                            = zeros(1,  size(pHs,1));
% pets(:, str2num(get(bHs(1,end),'String')))                  = 1;
% %
% % cflg
% if ~isempty(x.p4mpe.flg); 
%     im1                       	= umo_cstrs(char(x.p4mpe.flg(1,:)),[cflg{1,1},' '],  'im1');
%     % new addition:
%     if ~im1;                    im2                         = size(x.p4mpe.flg,2)+1;
%                                 x.p4mpe.pets                = [x.p4mpe.pets;pets];
%     % updating the existing 
%     else;                      	im2                         = im1;
%                                 x.p4mpe.pets(im1, :)        = pets;                                 end;
%  	x.p4mpe.flg(1:size(cflg,1), im2)                        = cflg;
% 	x.p4mpe.res(1:size(cres,1), im2)                        = cres;
% % the first set:
% else;                           im2                         = 1;
%                                 x.p4mpe.flg                 = cflg;
%                                 x.p4mpe.res                 = cres; 
%                                 x.p4mpe.pets                = pets;
%                                 x.p4mpe.mfg                	= [];
%                                 x.p4mpe.mfg                 = x.p4mpe.flg(1,:);                     end;
% %
% % updating x.p4mpe anyway (whether revision or not):
% x.p4mpe.pet_names               = g4iv2.yyy.cMat;
% set(gcf,    'UserData',x);
% %
% for i=2:1:size(bHs,2);
...
% set(bHs(:, 1),  'Enable','on');
% % marking the method with green:
% imx                             = umo_cstrs(char(get(bHs(:,1),'String')),[cflg{1,1},' '], 'im1');
% if sum(abs(get(bHs(imx(1), 1), 'BackgroundColor') - iv2_bgcs(6)),2)<10^-6;
%                                 set(bHs(imx(1), 1), 'BackgroundColor',iv2_bgcs(12));
% else;                           set(bHs(imx(1), 1), 'BackgroundColor',iv2_bgcs(6));                 end;
% return;
...
if isfield(x,'s4mpe');         
    ...
    set(gco,    'String','Quit',    'CallBack','delete(gcf);');                     return;         end;
%
% step 1 is done:
p4mpe                           = x.p4mpe;
...
disp([  ...
    ...
    '  TTCMC4 :  Constrained TTCM4 to get K1/k2 from the reference region',10,  ...
    ...
    '  Refernce regions are for calculation of BPND alone, except for TTCMC4',10,   ...
