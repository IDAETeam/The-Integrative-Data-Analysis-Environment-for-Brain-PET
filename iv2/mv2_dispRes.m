...
if umo_cstrs(char(x.s4mpe.ext_var{v-1}),'ratio', 'im1')>0;
                                s3{end+1}                   = '*SUVR';                              end;
if ~any(umo_cstrs(char(x.s4mpe.ext_var{v-1}),['K1 ';'k2 ';'k3 ';'k4 '],'im1')<1);
                                s3{end+1}                   = '*VT';                                end;
% 
...
%%
if get(findobj(gcf, 'Tag','disp.Res.R3C3'), 'Value')<2;                             return;         end;
hco                             = gco;
...
if c234s{2}(1)=='*';
    ...
    elseif strcmpi(c234s{2},'SUVR');
        ...
        ok(2)                   = max(im1);                                                 
    elseif strcmpi(c234s{2},'VT');
        adj_eq_0                = 'd1(:, 6)=d1(:,1)./d1(:,2).*(1+d1(:,3)./d1(:,4));';
        adj_eq                  = ' ';
        ok(2)                   = 7;                                                        end;    end;
%
if ~ok(2);                                                                          return;         end;
