...
if isempty(i1);
    opt                         = {'plot: XY plots ',                       ...
                                    'disp: histograms'                      ...
                                    'boxp: generates box plots',            ...
                                    'vudx: plot data vs. user-defined x',   ...
                                    'sysd: plots of systematic deviation',  ...
                                    'save: save data in Excel files',       ...
                                    'spmT: two-sample t-test (SPM)',        ...
                                    'spmP: paired t-test (SPM)',            ...
                                    'spmC: simple/multiple correlation',    ...
...
d0                              = zeros(size(gs,1),     length(pnos));
gis                             = deblank(dInfo(im1(1), 4:end));
for i=1:1:length(gis);          d0(gs==gis(i),  :)          = 1;                                    end;
d0(:)                           = d0.*pnos(ones(size(gs,1),1),  :);
di(:)                           = d0(:);
dix                             = di;
dix(~di,    :)                  = nan;
d(:)                            = d.*(dix(:, ones(1, size(d,2)))>0);
return;
%%
