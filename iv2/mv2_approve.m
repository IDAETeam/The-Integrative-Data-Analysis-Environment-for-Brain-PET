...
% To generate a file to note the output(s) was approved by the user for IDAE.iv2   
%
...
if isempty(i1);                 local_cbj(double(gcf),double(gco));               	return;         end;
if isempty(i2);                 local_inq(i1);                                      return;         end;
if length(i1)==1;               local_res(double(gcf),double(gco));                 return;         end;
...
if ~strcmpi(get(oNo, 'Tag'),'tag4_mv2_approve');                                    return;         end;
coUD                            = get(oNo,                  'userData');
if isempty(coUD);                                                                   return;         end;
% when called from VOILand > recording image positions
if strcmpi(get(fNo,'Name'),'VOILand');
    global g4vL2;
    coUD.sss{end+1}             = ['imagePos :  ',int2str(g4vL2{fNo}.inos)];                        end;
...
%%
if ~strcmpi(get(fNo, 'Tag'),'tag4_mv2_approve');                                    return;         end;
ud                              = get(fNo,                  'userData');
