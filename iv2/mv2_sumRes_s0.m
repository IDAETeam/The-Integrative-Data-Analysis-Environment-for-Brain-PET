...
function                        local_reset_for_voi_analyses(imax);
%%
global g4iv2;
x                               = get(gcf,      'UserData');
% .mpe is not set yet:
...
%
ic                              = 1;
% from VOI-analysis (RTMs):
f1                              = dir(fullfile(g4iv2.yyy.idx,'mpe','r4RTMs_*.mat'));
if numel(f1)>0;
    im1                         = umo_cstrs(['r4RTMs_',g4iv2.xxx(1).eza,'.mat'],char(f1.name), 'im1');
    for i=1:1:numel(f1);
        ic                      = ic + 1;
        fs{ic}                  = fullfile(f1(1).folder, f1(i).name);
        s1{ic}                  = ['VOI-analysis (RTMs/',f1(i).name(1, 8:end-4),')'];
        if im1(i)>0;
            s1{ic}              = [s1{ic}(1, 1:end-1),'-This package)'];            end;    end;    end;
%
% from map-analyses (RTMs);
f1                              = dir(fullfile(g4iv2.yyy.idx,'mpe','q4RTMs_*.mat'));

% from VOI-analyses (PIMs): 
f1                              = dir(fullfile(g4iv2.yyy.idx,'mpe','r4PIMs_*.mat'));
if numel(f1)>0;
    im1                         = umo_cstrs(['r4PIMs_',g4iv2.xxx(1).eza,'_', ...
                                    g4iv2.xxx(1).cpt,'.mat'], char(f1.name), 'im1');
    for i=1:1:numel(f1);
        ic                      = ic + 1;
        fs{ic}                  = fullfile(f1(1).folder, f1(i).name);
        s1{ic}                  = ['VOI-analysis (PIMs/',f1(i).name(1, 8:end-4),')'];
        if im1(i)>0;
            s1{ic}              = [s1{ic}(1, 1:end-1),'-This package)'];            end;    end;    end;
% from map-analyses (PIMs);
f1                              = dir(fullfile(g4iv2.yyy.idx,'mpe','q4PIMs_*_maps_*.mat'));
if numel(f1)>0;
    im1                         = umo_cstrs(['q4PIMs_',g4iv2.xxx(1).eza,'_maps_',g4iv2.xxx(1).cpt,  ...
                                    '_',g4iv2.xxx(1).snu,'.mat'], char(f1.name), 'im1');
    for i=1:1:numel(f1);
        ic                      = ic + 1;
        fs{ic}                  = fullfile(f1(1).folder, f1(i).name);
        us                      = strfind(f1(i).name,'_maps');
        s1{ic}                  = ['Map-analysis (PIMs/',f1(i).name(1, 8:us(1)-1),'/',   ...
                                    f1(i).name(1, us(1)+6:end-4),')'];
        if im1(i)>0;
            s1{ic}              = [s1{ic}(1, 1:end-1),'-This package)'];            end;    end;    end;
%
% mmm                             = {'RTMs', 'PIMs'};
% for k=1:1:double(isfield(g4iv2.xxx(1),'cpt'))+1'
% % 
...
for i=3:1:14;
    ...
    if i<=imax;
        ...
        set(findobj(gcf,'Tag',['disp.Res.R',int2str(i),'C2']),              ...
                                'Value',1,  'Style','pushbutton',   'String',' ', 'CallBack',' ');  end;
    set(findobj(gcf,'Tag',['disp.Res.R',int2str(i),'C3']),                  ...
                                'Value',1,  'Style','pushbutton',   'String',' ', 'CallBack',' ');
    set(findobj(gcf,'Tag',['disp.Res.R',int2str(i),'C4']),                  ...
                                'Value',1,  'Style','pushbutton',   'String',' ', 'CallBack',' ');
    set(findobj(gcf,'Tag',['disp.Res.R',int2str(i),'C5']),  'BackgroundColor',iv2_bgcs(0));         end;
return;
%%
...
%
s1{end+1}                       = '< back to approach selection';
%
...
%%
gco_v                           = get(gco,      'Value');
if gco_v<2;                                                                         return;         end;
% 
sss                             = get(gco,      'String');
x                               = get(gcf,      'UserData');
% rolling back to approach selection:
if sss{gco_v}(1)=='<';          
    set(gco,    'Value',1,  'String',x.mstr,    'CallBack','mv2_sumRes_s0(''method_selected'');');  
...
%
% adding DVR and SUVR to the variable lists, if applicable:
...
global g4iv2;
% setting C4 if q>0:
q                               = double(~strcmpi(get(findobj(gcf, 'Tag',           ...
                                    ['disp.Res.R',rns,'C1']),   'String'), 'y-data'));
q(:)                            = strcmpi(get(findobj(gcf, 'Tag','disp.Res.R3C1'), 'String'), 'vudx');
%
...
%
if strcmpi(s1,'plot:');
    local_reset_for_voi_analyses(6);
...
elseif strcmpi(s1,'disp:') || strcmpi(s1,'boxp:') || strcmpi(s1,'save:');
    local_reset_for_voi_analyses(4);
...
elseif strcmpi(s1,'vudx:');
    local_reset_for_voi_analyses(5);
    % setting for vudx:
...
% scanDB items to get size
sss                             = iv2_v4sdb(1);
s1{1}                           = 'Available x variables:';
for i=1:1:size(c2,1); 
    im1                         = umo_cstrs(lower(char(sss(:,1))), [lower(c1(i, :)),' '], 'im1');
    d                           = gei(g4iv2.yyy.ifl,    sss{im1(1),2});
    s1{end+1}                   = ['- ',deblank(c1(i,:)),'(:,1): ',c2(i, :)];
    for j=2:1:size(d,2);        
        s1{end+1}               = ['- ',deblank(c1(i,:)),'(:,',int2str(j),'): '];               end;    end;
%
s1{end+1}                       = '? variable not found? Define it in scanDB.m with #';
...
x                               = get(gcf,  'UserData');
%
ok                              = zeros(size(ddd,1),    1);
d_lines{1}                      = '% data lines';
ic                              = 1;
for i=1:1:size(ddd,1);
    if strcmpi(ddd{i,1},'vudx');
        ic                      = ic + 1;
        d_lines{ic}             = '% user-defined x:';
        ic                      = ic + 1;
        d_lines{ic}             = lolca_convert_vudx(ddd(i, :));
    elseif strcmpi(ddd{i,1},'2nd var');
        ic                      = ic + 1;
        d_lines{ic}             = ['% ',ddd{i,    1}];
        ic                      = ic + 1;
        [d_lines{ic-2}, d_lines{ic}]                        ...
                                = local_convert_2nd_var(ddd(i-1:i,:), vnm(i-1:i));
    else;
        ic                      = ic + 1;
        d_lines{ic}             = ['% ',ddd{i,    1}];
     	ic                      = ic + 1;
        y                      	= load(x.mpe{ddd{i,2}(1)});
        d_lines{ic}             = ['res ', deblank(y.s4mpe.ffg(ddd{i,2}(3), :)), ' ',ddd{i,5},  ...
                                    ' ',  vnm{i}, ' ',deblank(ddd{i,4})];                           end;
    ok(i, :)                    = ~isempty(d_lines{ic});                                            end;
...
function    [i_line, j_line]    = local_convert_2nd_var(dij, vij);
%%
x                               = get(gcf,  'UserData');
% variable names & their acronyms (=c1 with ':' at the end): 
q                               = iv2_sumRes_beta('convert',[]);
[c1, c2]                        = getLseg(char(q),  	1);
c1(c1==':')                     = '@';
im1                             = umo_cstrs(c2, dij{2,4},   'im1');
%
if size(str2num(dij{1,5}),2)==1 && size(str2num(dij{2,5}),2)>1;
    di5                         = repmat([int2str(str2num(dij{1,5})),','],1, size(str2num(dij{2,5}),2));
    dij{1, 5}                   = di5(1,    1:end-1);
elseif size(str2num(dij{1,5}),2)~=size(str2num(dij{2,5}),2);
    j_line                      = [];                                          	return;             end;
%
y                               = load(x.mpe{dij{1,2}(1)});
i_line                          = ['res ', deblank(y.s4mpe.ffg(dij{1,2}(3), :)), '  ',dij{1,5},     ...
                                    ' ', c1(im1(1), :), vij{1}, ' ',deblank(dij{1,4})]; 
%
y                               = load(x.mpe{dij{2,2}(1)});
j_line                          = ['res ', deblank(y.s4mpe.ffg(dij{2,2}(3), :)), ' ',dij{2,5},      ...
                                                            ' ',  vij{2}, ' % ',deblank(dij{2,4})]; 
return;
%%

function    d_line              = lolca_convert_vudx(ddi);
%%
% example d_line: res vudx 2,3,4  PK
%
d_line                          = [];

ddi{3}(ddi{3}=='(' | ddi{3}==':' | ddi{3}==',' | ddi{3}==')' | ddi{3}==':')         = ' ';
c12_3                           = getLseg(ddi{3},   1:2);
%
cm1                             = umo_cstrs(c12_3(1).mat, [], 'cm1');
if sum(cm1(:,2)>0)~=1;
    disp('.error in vudx line - one variable alone is allowed');
    local_blink(findobj(gcf, 'Tag','disp.Res.R3C3'));                               return;         end;
%
% translating the variable name in sumRes.m (short version) to the long variable name:  
v4sdb                           = iv2_v4sdb(1);
im1                             = umo_cstrs(lower(v4sdb(:,1)), lower(c12_3(1).mat(1,:)),   'im1');
%
d0                              = 'res vudx ';
for i=1:1:size(ddi{3},1);      	d0                          = [d0,deblank(c12_3(2).mat(i,:)),','];  end;
d_line                          = [d0(1, 1:end-1),' ',c12_3(1).mat(1, :),' ',v4sdb{im1(1),2}];
return;
%%
...
while 1;
    rc                          = rc + 1;
    h                           = findobj(gcf, 'Tag',['disp.Res.R',int2str(rc),'C1']);
    if isempty(h);                                                                  break;          end;
    if ~any(get(h, 'String')~=' ');                                                 break;          end;
    L2c(1,  rc)                 = get(findobj(gcf, 'Tag',['disp.Res.R',int2str(rc),'C2']),'Value')>1;
    L2c(2,  rc)                 = strcmpi(get(h, 'String'),'vudx');                                 end;
%
L2u                             = sum(L2c, 1);
ic                              = 0;
for i=find(L2u>0);
    ...
    ddd{ic, 1}                  = get(findobj(gcf, 'Tag',['disp.Res.R',int2str(i),'C1']), 'String');
    %
    % checking RxC2 - if ok recording its user data:
    hRxC2                       = findobj(gcf, 'Tag',['disp.Res.R',int2str(i),'C2']);
