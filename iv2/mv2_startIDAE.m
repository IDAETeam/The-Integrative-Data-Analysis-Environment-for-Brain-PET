...
pOK                             = 1;
disp(['.checking presence of PET source files: ',g4iv2.xxx(1).pid]);
for i=1:1:fbc(3);
    disp([' PET #',int2str(i),' (',deblank(g4iv2.yyy.cMat(i, :)),'):']);
    p0                          = gei(i2,                   ['dx4c',intstr(i,3)]);
    p0(p0=='/' | p0=='\')       = filesep;
    for j=1:1:size(p0,1);       pet{j}                      = fullfile(stm,deblank(p0(j,:)),add.pet);
                                res{j}                      = fullfile(stm,deblank(p0(j,:)),add.res);
                                psid{j}                     = '???';                                end;
    % scans with the 'dmy' folder: 
    imex                        = umo_cstrs(p0ex,char(pet), 'im1');
    ppp                         = zeros(size(imex));
    %
    for j=find(~imex)';
    ...
    if any(ppp==1);
        disp(' .info: no source files yet for: ');
        dispCharArrays(2,g4iv2.yyy.snm(ppp==1, :),2,char(pet(ppp==1)));
        disp(' >need to prepare source files, as scans are done');
        disp(['  or if no scans, replace folders with: ',p0ex]);                                	end;
    %
...
if ~mOK | ~pOK;
    ...
    disp(' fix the problem and resubmit');
