...
[q, is]                         = sort(p(:, 2));
v                               = round(sort([get(h(1),'Value'),  get(h(2),'Value')]).*g4vL2{fNo}.cmd);
if isfield(g4vL2{fNo},'mean_gm');
    v(2)                        = g4vL2{fNo}.iM(g4vL2{fNo}.mean_gm_pos(1));                         end; 
if v(2)<v(1);                   v(1)                        = v(2)./3;                              end;
v(v<1)                          = 1;
v(v>g4vL2{fNo}.cmd)             = g4vL2{fNo}.cmd;
set(h(is(2)),   'Value',v(2)./g4vL2{fNo}.cmd);
set(h(is(1)),   'Value',v(1)./g4vL2{fNo}.cmd);
%
...
p3                              = cell2mat(get(h3,  'Position'));
[q, is3]                        = sort(p3(:, 2));
set(h3(is3(2)),     'String',int2str(v(2)));
set(h3(is3(1)),     'String',int2str(v(1)));
