...
if im_rtga(1)>0;               	k2Rs{im_rtga(1)}         	= deblank(p4mpe.flg{4,im_rtga(1)}); 
    ...
    if im_mrtm2>0 && umo_cstrs(char(p4mpe.flg{4,im_mrtm2}),'fix','im1')>0;
                                k2Rs{im_mrtm2(1)}           = k2Rs{im_rtga(1)};                     end;
    im_strm2                    = umo_cstrs(char(p4mpe.flg(1,:)),'SRTM2 ',  'im1');
    if im_strm2>0 && umo_cstrs(char(p4mpe.flg{4,im_strm2}),'fix','im1')>0;
                                k2Rs{im_strm2(1)}           = k2Rs{im_rtga(1)};             end;    end
%
global g4iv2;
