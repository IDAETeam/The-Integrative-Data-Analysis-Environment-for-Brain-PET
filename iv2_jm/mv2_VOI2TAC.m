...
if exist(x.Info4TACs.voimat,'file');
        y                       = load(x.Info4TACs.voimat);
else;   disp('> problem! VOI info file no longer located');
        disp([' sought: ',x.Info4TACs.voimat]);                                     return;         end
% x.Info4TACs.voimat
% p.p2m(imp(1))
%
...
disp(['            dynamic PET: ',datestr(ezm_max)]);
...
%     if mv2_get_dnum(ooo(1))>mv2_get_dnum(ooo(3));
