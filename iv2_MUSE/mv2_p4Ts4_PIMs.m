...
%%
ic                              = 0;
% looping over treatments of the vascular volume:
...
function  [o1, o2, o3, o4, r2] 	= local_qqq_ttcm4c(flg,res,c0,inm);
%%                                
% k2 will be replaced by K1/k2 from reference region:
eic                             = 'ereexi';
ic                              = 0;
% looping over treatments of the vascular volume:
for i=1:1:size(flg{2},1);
    ...
    for j=1:1:size(flg{3},1);
        ...
        for k=1:1:size(flg{4},1);
            ic                  = ic + 1;
            o2{ic}              = {flg{1}, eic, deblank(flg{3}(j,:)), deblank(flg{4}(k,:)),  'VT'};
            o1{ic}              = [inm,'_',o2{j}{2},'_',o2{j}{3},'_',c0,'_',deblank(flg{4}(k,:)),'.ezd'];
            o3{ic}              = [flg{1},' / ',eic,' / ',o2{j}{3},' / input#',int2str(k),' / VT'];
            r2{ic}              = {flg{1}, res{2}(i,:), res{3}(j, :), res{4}(k, :), nan};
                                                                                    end;    end;    end;
%
