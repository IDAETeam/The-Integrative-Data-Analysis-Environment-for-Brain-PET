...
%                   val{1} = '1st.xyz'; val{2} = '2nd.xyz' will roud both
%                   and show 1st if both in voxles.
...
%   'exd',val   -   To perform eval(val) when exit from a VOILand session
%                   (exd for exit_do)
%   'iv2','on'  -   To show 'starting VOILand. be patient..' on the
%                   information bord of Level 2 Window of IDAE.iv2.
%
%
% (cL)2012~22    hkuwaba1@jhmi.edu 
...
if xyzflg && ~iscell(xyzval) && exist(xyzval,'file')~=2
    disp('Unable to locate input file of ''xyz'' option (aborting)');
    disp([' entered: ',xyzval]);                                                    return;         end;

Vs                              = findobj(groot,    'Name','VOILand', 'Tag',sidval);
if ~isempty(Vs);                figure(Vs(1));                                      return;         end;
%
%%
if strcmpi(iv2val,'on');
    h_L2W_R0                   	= findobj(findobj(groot,'Tag','iv2L2W'), 'Tag','L2W_gUseR0');
    str_L2W_R0               	= get(h_L2W_R0,    'String');
    bgc_L2W_R0               	= get(h_L2W_R0,    'BackgroundColor');
    set(h_L2W_R0,  'String','Starting VOILand. Be patient ..', 'BackgroundColor',iv2_bgcs(11));    	end;
%
drawnow;
...
%% when L2W_RO is used:
if strcmpi(iv2val,'on');
    set(h_L2W_R0,   'String',str_L2W_R0,  'BackgroundColor',bgc_L2W_R0);                            end;
...
jstrs                           = {'vL2_BJs([],[]);','vLx_VJs(''select'',gco);',        ...
                                ' ',' ','vLx_VJs(''done'',gco);',                       ...
                                'vL2_BJs(''stOvr'',gco);','vL2_VJs(''vOLs'',gco);',     ...
                                'vL2_modifyVOIs(''set'',gco);'};
