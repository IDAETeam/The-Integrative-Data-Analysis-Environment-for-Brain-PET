function                        cropMR_muse(iii,ooo);
%% crop MUSE-derived VOI mask & MRIs (prepMPfsx, etc):
% modified from local_831@use_freeSurfer.m
...
if ddo(1)<min(ddi(1:3)) && min(ddo([2:3,5]))>max(ddi(1:3));
                                performed                 	= 1;
                                MUSE_cropMRIs({iii{3},ooo{3}},iii{1},ooo{1},  'fs81nii_v2');
...
if max(ddo(1:3))<min(ddi(1:3)); performed                 	= 1;
                                MUSE_cropMRIs(iii(1:3),iii(4),ooo(1:3),   'crop_own');                
...
if ddo(5)<ddo(1);               performed                 	= 1;
                                MUSE_cropMRIs(ooo{1},ooo{3},ooo(5),       'mask2ezr');
...
%
vi                              = consolidVOINos([90000; 90400], vnosets_muse('mus'));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% use the modified function MUSE_cropMRIsbelow for MUSE
function    MUSE_cropMRIs(i1,i2,i3,i4); 
...
%
%   'like_AAL'  to use *_fs81-msk.nii (= standard FS81 VOI transferred to
%               individual's MRI space; thus like_AAL; voxel values =
%               IDAE's VOIID#s)
%
...
%% crop muse outputs by its own 'brain' estimates 
% disable the local_xzy within itself now, as muse does not need re-orientation
disp('..local_crop_own');
% return;
v1                              = spm_vol(iii{1});
vM                              = spm_read_vols(v1);
...
%     s12_resample(vy,v1,[0,0],  ooo{i});                                                            end;
%
...
function                        local_fs81nii_v2(mmm,fsout,musenii);
...
%
s12_resample(v0,v1x,[0,0],      musenii);
delete(v1x.fname);
return;
%%
...
disp('..local_mask2ezr');
vfln                            = which('MUSEColorLUT.m');
if isempty(vfln);               disp('.error! unable to locate muse VOI LUT');        
                                disp(' sought: MUSEColorLUT.m');              return;         end;
c                               = umo_getptf(vfln,  0,1:3);
...
disp('.done! (shared MUSE VOI file, v.MUSE)');
disp([' output: ',ooo{1}]);
%
return;
%%

function                        local_like_aal(iii,fbc,ooo);
%%
% #1   mri/tmp_fs81_msk.nii
% #2   mri/tmp.nii
% #3   mri/tmp_R1.nii
% #4   mri/tmp_gmOLs.xyz
% #5   mri/tmp_fs81_msk_ok.txt
% %
% $1   mri\fsbc_fs81.nii
% $2   mri\fsbc.nii
% $3   mri\fssz.nii
...
global g4iv2;
disp('.replacing Freesurfer outputs by AAL-like approach');
...
vM(:)                           = spm_read_vols(v1);
vM(isnan(vM))                   = 0;
xyz                             = xyz2n(find(vM(:)>0),  v1.dim);
% extracting 
m                               = round(12./sqrt(sum(v1.mat(1:3, 1:3).^2)));
[x1, y1, z1]                    = ndgrid(   ...
                             	[max([1,min(xyz(:,1))-m(1)]), min([v1.dim(1),max(xyz(:,1))+m(1)])],	...
                             	[max([1,min(xyz(:,2))-m(2)]), min([v1.dim(2),max(xyz(:,2))+m(2)])],	...
                                [max([1,min(xyz(:,3))-m(3)]), min([v1.dim(3),max(xyz(:,3))+m(3)])]);
clear xyz;
%
G1                              = ones(4,   prod(size(x1)));
G1(1:3, :)                      = [x1(:), y1(:), z1(:)]';
G2                              = G1;
for i=1:1:3;                    G2(i,   :)                  = G1(i, :) - min(G1(i,:)) + 1;        	end;
M2                              = (G2'\(G1'*v1.mat'))';
M2([2:5,7:10,12])               = 0;
v2                              = dimmat(G2(1:3,end)', sqrt(sum(v1.mat(1:3, 1:3).^2)),  'mat',M2);
% resampling fs81_msk.nii
if mv2_get_dnum(ooo(1))<mv2_get_dnum(iii(1));
                                s12_resample(v2,v1,[0,0],   ooo{1});        
% resampling MRIs
    for i=2:1:3;              	s12_resample(v2,spm_vol(iii{i}),    [0,0],  ooo{i});                end;
else;                           disp('> VOI mask & MRI - previously cropped');                    	end;
%
if mv2_get_dnum(ooo(4))<mv2_get_dnum(ooo(1));
    xyz                     	= ged(iii{4},   1);
    ...
    %
    G0(:)                     	= M2\(v1.mat*G0);
    um_save(ooo{4},G0(1:3,:)',  struct('h2s',32,'c',mfilename,'p',iii{4},'cp','m'),[],      ...
                                'imagesize',max(G2(1:3, :),[],2)',  'M0_original',v1.mat,   ...
                                'M10_cropped',M2,       'M1_cropped',M2);
                                disp('> GM outlines - cropped');
else;                           disp('> GM outlines - previously cropped');                         end;
%
if mv2_get_dnum(ooo(5))<mv2_get_dnum(ooo(1));
                                msk2ezr(ooo{1}, ooo{5});
                                disp('> FS81 VOI file - generated');
else;                           disp('> FS81 VOI file - previously done');                          end;
%
if mv2_get_dnum(ooo(6))<mv2_get_dnum(ooo(5));
                                fs_f81Tf45(ooo{5},  ooo{6});
                                disp('> FS45 VOI file - generated');
else;                           disp('> FS45 VOI file - previously done');                          end;
return;
%%