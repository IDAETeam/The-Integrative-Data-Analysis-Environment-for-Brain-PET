...
for i=find(~im1)';              
    v0(consolidVOINos(vnosets_muse(lower(vvv{i})),[]),:)         = 1;
    ic                          = ic + 1;
...
for i=find(~im1)';              
    vi                          = consolidVOINos(vois4iv2.vnos(:,1),    ...
                                                            consolidVOINos(vnosets_muse(lower(vvv{i})),[]));
    ss                          = [ss, v2a{i},' &'];
    ic                          = ic + 1;
...
%
v0                              = zeros(99999, 1);
for i=1:1:numel(v2b);           v0(consolidVOINos(vnosets_muse(vv2{i}),[]), :)       = 1;                end;
%
vois4iv2.vnos                   = zeros(sum(v0>0),          numel(vois4iv2.regVRs)+1);
vois4iv2.vnos(:,    1)          = find(v0>0);
vv                              = VOIdef(vois4iv2.vnos(:,1));
...
for i=1:1:numel(v2b);            
    vi                          = consolidVOINos(vois4iv2.vnos(:,1),    ...
                                                            consolidVOINos(vnosets_muse(lower(vv2{i})),[]));
